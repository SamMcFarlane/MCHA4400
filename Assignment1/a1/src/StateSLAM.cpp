...
Eigen::VectorXd StateSLAM::dynamics(const Eigen::VectorXd & x) const
{
    ...
    Eigen::Vector3<double> Thetanb = x.segment<3>(9);
    Eigen::Vector3<double> vBNb = x.segment<3>(0);
    Eigen::Vector3<double> omegaBNb = x.segment<3>(3);

    f.segment<3>(6) = rpy2rot(Thetanb)*vBNb;
    f.segment<3>(9) = rpy2TK(Thetanb)*omegaBNb;
...
Eigen::VectorXd StateSLAM::dynamics(const Eigen::VectorXd & x, Eigen::MatrixXd & J) const
{
    ...
    //Eigen::VectorX<autodiff::dual> fvar;

    //Eigen::VectorX<autodiff::dual> x_dual = x.cast<autodiff::dual>();

    //J = jacobian([&](const Eigen::VectorXd& x) -> Eigen::VectorXd {return dynamics(x);},wrt(x_dual),at(this, x_dual),fvar);
...
Eigen::VectorXd StateSLAM::predictFeatureBundle(const Eigen::VectorXd & x, Eigen::MatrixXd & J, const Camera & cam, const std::vector<std::size_t> & idxLandmarks) const
{
    ...
    for (std::size_t i = 0; i < nL; ++i)
    {
        ...
        //std::cout<<"STATESLAM.cpp****************** h = "<<h<<std::endl;
  
        J.block(2 * i, 0, 2, nx) = Jfeature;
       // std::cout<<"\nSTATESLAM.cpp****************** J = "<<J<<std::endl;
    }
    return h;
}
...
Eigen::VectorXd StateSLAM::StateSLAMPoseLandmarks(const Eigen::VectorXd & x, Eigen::MatrixXd & J, const Camera & cam, std::size_t idxLandmark) const{

    Eigen::VectorX<autodiff::dual> fvar;

    Eigen::VectorX<autodiff::dual> x_dual = x.cast<autodiff::dual>();

    auto Lambda = [&](const Eigen::VectorX<autodiff::dual> & x_dual) {
        return StateSLAMPoseLandmarks(x_dual, cam, idxLandmark);
    };

    J = jacobian(Lambda, wrt(x_dual), at(x_dual), fvar);

    return fvar.cast<double>();
}


Eigen::VectorXd StateSLAM::predictSLAMPoseLandmarks(const Eigen::VectorXd & x, Eigen::MatrixXd & J, const Camera & cam, const std::vector<std::size_t> & idxLandmarks) const
{
    const std::size_t & nL = idxLandmarks.size(); // This should now extract the measurement and it's jacobian
    const std::size_t & nx = size();
    assert(x.size() == nx);

    Eigen::VectorXd h(8*nL); //Measurement is going to be 8*num_Landmarks (x,y coords for each corner per tag) 
    J.resize(8*nL, nx);
    for (std::size_t i = 0; i < nL; ++i)
    {
        Eigen::MatrixXd Jfeature;
        Eigen::VectorXd rQOi = StateSLAMPoseLandmarks(x, Jfeature, cam, idxLandmarks[i]);
